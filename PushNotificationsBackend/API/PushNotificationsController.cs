...
namespace PushNotificationsBackend.API
{
	[RoutePrefix("api")]
	public class PushNotificationsController : ApiController
	{
		...
		[Route("")]
		[HttpGet]
		public IHttpActionResult GetAllRegisteredDevices() => this.Ok(this.db.RegisteredDevices);
		...
		[Route("register")]
		[HttpPost]
		public async Task<IHttpActionResult> RegisterOrUpdateDevice([FromBody] DeviceInformation deviceInfo)
		{
			if (deviceInfo == null)
			{
		...
		[HttpDelete]
		public async Task<IHttpActionResult> UnregisterDevice(string uniqueId)
		{
			...
			await this.notificationHubClient.DeleteInstallationAsync(uniqueId).ConfigureAwait(false);
